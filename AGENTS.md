...
Follow-up 13: If the far-right screen gutter shows the movie backdrop where the page scrollbar should be, do not fake-fill it with `#itemDetailPage::after`. Restore the real detail-page scrollbar instead: scope `scrollbar-color` and `::-webkit-scrollbar-track/thumb` to `html:has(#itemDetailPage)`, `body:has(#itemDetailPage)`, `#reactRoot`, and `.mainAnimatedPages` so the reserved scrollbar gutter paints as `#050505` with a subtle visible thumb.
