...
Follow-up 13: If the far-right screen gutter shows the movie backdrop, check root/body padding before adding masks or scrollbar styling. In this theme the gap was caused by right padding on the page root/body path. The working direction is to scope `margin-right: 0` and `padding-right: 0` to `body:has(#itemDetailPage)`, `#reactRoot`, and `.mainAnimatedPages`; do not force a custom detail-page scrollbar unless the user explicitly wants that UI back.
